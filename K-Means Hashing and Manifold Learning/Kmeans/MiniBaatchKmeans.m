%% Kmeans in batches and update with online mechanism
%% And compute the distances
function [centroids, idx,dists,iter] = MiniBatchKmeans(X, k,max_iter,method,eps)
if ~exist('max_iter','var'), max_iter=3000; end
if ~exist('eps','var'), eps=0.00005; end
...
while (error>eps) && (iter<max_iter)
    ...
    [idx,dists] = FindClosestCentroids(X, centroids);
