...
function TDA=Mapper(X,res,overlap,buckets,lens_result,labels,name) 
  if ~exist('name','var'), name='x'; end
  [m,n] = size(X);
  matrix=X;
  ...
  if exist('labels','var')
      TDA.label_averages=cell2mat(label_averages)';
      node_table=[node_table,cell2mat(label_averages)'];
  end
  s=sprintf('graphs/Mapper/Mapper_%s_%.2f_%.2f_%.2f_nodes.csv',name,res,overlap,buckets);
  fid = fopen(s, 'w');
  fprintf(fid, 'Id,NSize,LensAverage,LabelAverage\n');
  fclose(fid);
  dlmwrite(s, node_table, '-append', 'precision', '%.6f', 'delimiter', ',');
  
  %% Create graph
  [r,c]   = find(TDA.adj~=0);
  v       = nonzeros(TDA.adj);
  compact = [r,c,v];
  s=sprintf('graphs/Mapper/Mapper_%s_%.2f_%.2f_%.2f_edges.csv',name,res,overlap,buckets);
  fid = fopen(s, 'w');
  fprintf(fid, 'Source,Target,Weight\n');
  fclose(fid);
  dlmwrite(s, compact, '-append', 'precision', '%.6f', 'delimiter', ',');
