		...
		if e > 0:
			...
			if opts["2grams"]:
				...
				for i in xrange(len(features)-2):
					features.append((hash(str(features[i][0])+str(features[i+1][0])+str(features[i+2][0]))%opts["D"],1))
				for i in xrange(len(features)-3):
					features.append((hash(str(features[i][0])+str(features[i+1][0])+str(features[i+2][0])+str(features[i+3][0]))%opts["D"],1))
...
if __name__ == "__main__":
	...
	opts["n_passes"] = 180 # Maximum number of passes to run before halting
